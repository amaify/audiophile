...
const icon = (type: AlertType) => {
  if (type === AlertType.Success) return <CheckCircleIcon className="w-8 h-8 " aria-hidden="true" />;
  if (type === AlertType.Warning) return <ExclamationCircleIcon className="w-8 h-8 " aria-hidden="true" />;
  if (type === AlertType.Error) return <XCircleIcon className="w-8 h-8 " aria-hidden="true" />;
  return "";
};

export const Alert = ({ message, type }: Props) => {
  return (
    <div className={`relative bg-white shadow rounded-md overflow-hidden border border-${color(type)} mb-[20px] p-6`}>
      <div
